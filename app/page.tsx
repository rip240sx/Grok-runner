...
export default function Game() {
  ...
  const Joystick = ({ onMove }: { onMove: (v: { x: number; y: number }) => void }) => {
    ...
    return (
      <div style={{ position: "fixed", bottom: 80, left: 20, width: 100, height: 100, background: "rgba(0,0,0,0.3)", borderRadius: "50%", touchAction: "none" }}
        ...
        onTouchMove={(e) => { if (!touch.current.active) return; const t = e.touches[0]; const dx = (t.clientX - touch.current.x) / 50; const dy = (t.clientY - touch.current.y) / 50; onMove({ x: Math.max(-1, Math.min(1, dx)), y: 0 }); }}
  ...
  if (gameScreen === "menu") {
    return (
      <div style={{ position: "fixed", inset: 0, background: "#0b1114", color: "white", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
        <h1 style={{ fontSize: 48 }}>GROK RUN</h1>
        <button onClick={startGame} style={{ padding: "15px 40px", fontSize: 24, background: "#4CAF50", color: "white", border: "none", borderRadius: 10 }}>PLAY</button>
        <button onClick={() => setGameScreen("howToPlay")} style={{ padding: "10px 30px", fontSize: 18, background: "#333", color: "white", border: "none", borderRadius: 10 }}>How to Play</button>
      </div>
    );
  }
  ...
  if (gameScreen === "howToPlay") {
    return (
      <div style={{ position: "fixed", inset: 0, background: "#0b1114", color: "white", padding: 40, textAlign: "center" }}>
        <h2>How to Play</h2>
        <p>Use joystick to move</p>
        <p>Press JUMP to jump</p>
        <p>Collect microchips</p>
        <p>Avoid viruses</p>
        <p>Reach wormhole to win!</p>
        <button onClick={() => setGameScreen("menu")} style={{ marginTop: 20, padding: "10px 30px", background: "#666", color: "white", border: "none", borderRadius: 10 }}>Back</button>
      </div>
    );
  }
  ...
  if (gameScreen === "gameOver") {
    return (
      <div style={{ position: "fixed", inset: 0, background: "#0b1114", color: "white", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
        ...
        <button onClick={startGame} style={{ padding: "15px 40px", background: "#4CAF50", color: "white", border: "none", borderRadius: 10 }}>RETRY</button>
        <button onClick={() => setGameScreen("menu")} style={{ padding: "10px 30px", background: "#666", color: "white", border: "none", borderRadius: 10 }}>MENU</button>
      </div>
    );
  }

  return (
    <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "calc(var(--vh) * 100)", overflow: "hidden", background: "#0b1114", touchAction: "none" }}>
