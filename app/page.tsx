...
export default function Game() {
  ...
  const playSoundEffect = useCallback((type: "jump" | "coin" | "hit" | "wormhole") => {
    ...
    if (type === "jump") { osc.frequency.value = 400; osc.frequency.exponentialRampToValueAtTime(600, ctx.currentTime + 0.1); gain.gain.setValueAtTime(0.3, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.1); }
    if (type === "coin") { osc.frequency.value = 800; osc.frequency.exponentialRampToValueAtTime(1200, ctx.currentTime + 0.05); gain.gain.setValueAtTime(0.4, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.1); }
    if (type === "hit") { osc.type = "sawtooth"; osc.frequency.value = 200; osc.frequency.exponentialRampToValueAtTime(50, ctx.currentTime + 0.2); gain.gain.setValueAtTime(0.5, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.2); }
    if (type === "wormhole") { osc.type = "sine"; osc.frequency.value = 400; osc.frequency.exponentialRampToValueAtTime(1000, ctx.currentTime + 0.3); gain.gain.setValueAtTime(0.4, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.3); }
    osc.start(); osc.stop(ctx.currentTime + (type === "hit" || type === "wormhole" ? 0.3 : 0.1));
  }, []);
  ...
  useEffect(() => {
    ...
    const drawCharacter = (ctx: CanvasRenderingContext2D, p: typeof player.current, animTimer: number) => {
      ...
      if (grokImg.complete && grokImg.naturalWidth) {
        ctx.drawImage(grokImg, -p.w / 2, -p.h / 2, p.w, p.h);
      } else {
    ...
    const loop = (now: number) => {
      ...
      if (!gs.gameOver && !gs.transitioning) {
        ...
        if (Math.abs(inputX) > 0.1) { p.vx += inputX * 600 * dt; p.facingRight = inputX > 0; }
        else p.vx *= 0.8;
        if (jumpRef.current && p.grounded) p.vy = -550;
        ...
        for (const seg of floorSegments.current) {
          if (p.x > seg.x && p.x < seg.x + seg.w && p.y + 30 > groundY && p.y + 30 < groundY + 30 && p.vy > 0) {
            p.y = groundY - 30; p.vy = 0; p.grounded = true;
          }
        }
        ...
        if (p.y > canvasH + 100) { gs.lives--; if (gs.lives <= 0) gs.gameOver = true; else { p.x = 100; p.y = groundY - 100; p.vx = 0; p.vy = 0; } }
  ...
  if (gameScreen === "howToPlay") return <MenuScreen onStart={startGame} onHowToPlay={() => {}} />; // Replace with real HowToPlay later
